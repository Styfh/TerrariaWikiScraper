...
class ItemExtractor(Thread):
                    ...
                    for stat in table.find('tbody').find_all('tr'):
                        ...
                        for word in header_words:
                            if(word == "type"):
                                ...
                                tags_container = stat.td.find('div', class_="tags")
                                
                                tags = None
                                
                                if(tags_container):
                                    tags_html = tags_container.find('span')
                                    if(tags_html):
                                        for tag in tags_html:
                                            # print(tag.text)
                                            text = tag.text.strip().lower()
                                            if("npc" in text or "enemy" in text or "boss" in text):
                                                return
                                ...
                                tdValue = stat.td.text
                                tdNum = re.search(r'\d+', tdValue)
                                
                                if(tdNum):
                                    tdValue = int(tdNum.group())                                
                                    statTable[stat.th.text] = int(tdValue)
                                else:
                                    statTable[stat.th.text] = tdValue
    ...
    def getInfoFromPage(self, response):
        ...
        # Iterate through all tables in the page
        for table in craftsTables: 
            headers = []
            header_cells = table.find_all('th')
            
            available_headers = ["station", "ingredient", "result"]
                ...
                if count > 1:
                    ...
                    for row in rows:
                        ...
                        if headers:
                            ...
                            if(self.checkMissingRows(currMissingHeaders)):
                                for header in currMissingHeaders:
                                    header[2] = False
                            
                            for j in range(0, len(currHeaders)):
                                # print(total, currHeaders[j][1], currHeaders[j][0])
                                total += currHeaders[j][1]
                                if("result" in currHeaders[j][0]):
                                    result_index = (total - currHeaders[j][1], currHeaders[j][1])
                            
                            # print("RESULT INDEX: ", result_index)
                            
                            if(result_index[0] > -1):
                                if(not self.checkMissingRows(currMissingHeaders)):
                                    # print("RESULT: ", datas[result_index[0]])
                                    if(datas[result_index[0]].has_attr('rowspan')):
                                        result_rowspan = int(datas[result_index[0]]['rowspan'])
                            
                            # print("RESULT ROWSPAN: ", result_rowspan)
                            
                            currCol = None
                            total = 0
                            
                            # print("ROW DATA: ", datas)
                            ...
                            for i in range(0, len(datas)):
                                ...
                                self.findItemImages(datas[i])
                                
                                rowspan = 1
                                if(datas[i].text.strip().lower() != 'or'):
                                    if(datas[i].has_attr('rowspan')):
                                        rowspan = int(datas[i]['rowspan'])
                                    
                                    
                                    if(rowspan < result_rowspan and not self.checkMissingRows(currMissingHeaders)):
                                        currMissingHeaders.append([currCol, result_rowspan - rowspan, True])
                                    
                                    for header in currMissingHeaders:
                                        exists.append(header)
                                        if(header[1] <= 0):
                                            currMissingHeaders.remove(header)
                                        elif(header in exists):
                                            currMissingHeaders.remove(header)
                                    
                                    # print("CURRENT COLUMN: ", currCol)
                                    # If it there isn't any columns missing, or if it's the last 
                                    if(not self.checkMissingRows(currMissingHeaders)):
                                        total = 0
                                        idx = i
                                        for j in range(0, len(currHeaders)):
                                            # print(f"TOTAL: {total}, IDX: {idx}, J: {j}")
                                            total += currHeaders[j][1]
                                            if(total >= idx + 1):
                                                # print("This is curr header", total, currHeaders[j][0])
                                                currCol = currHeaders[j]
                                                break
                                    else:
                                        for header in currMissingHeaders:
                                            if(header[2] == False):
                                    ...
                                    # print("CURRENT COLUMN: ", currCol)
                                    # print("DATA: ", datas[i].text.strip())
                                    
                                    # print(f"CHECKING MISSING ROWS: {rowspan}, {result_rowspan}")
                                    
                                    # print(currCol, "ALREADY IN MISSING: ",column_in_missing)
                                        
                                    # print("CURRENT MISSING HEADERS: ", currMissingHeaders)
                                    ...
                                    for word in header_syn:
                                        if "result" in word:
                                            ...
                                            if(result_rowspan > 1):
                                                ...
                                                for j in range(1, currCol[1]):
                                                    print(i, j)
                                                    name = name + datas[i + j].text
                                                    if(not link):
                                                        link = datas[i + j].find('a')
                                            ...
                                            stations = datas[i].find_all('span', class_='i')
                                
                                            if not stations:
                                                # print("not stations", datas[i].text.strip())
                                                currItem.addStation(datas[i].text.strip())
                                            else:
                                            ...
                                            if(ingredients):
                                                ...
                                                ingredients = datas[i].text.strip()  
                        ...
                        if(currItem.getName() and len(currItem.getIngredients()) != 0 and len(currItem.getStation()) != 0 and not self.checkMissingRows(currMissingHeaders)):
